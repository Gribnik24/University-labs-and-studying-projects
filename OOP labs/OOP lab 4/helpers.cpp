...
double iqr_c(const double* x, int n) {
    ...
    // Динамический массив-копия данных
    double* v = static_cast<double*>(std::malloc(sizeof(double) * n));
    if (!v) return 0.0;
    ...
    double res = quant(0.75) - quant(0.25);
    std::free(v);
    return res;
}
...
void build_empirical_density_c(const double* sample, int n, int bins, EmpiricalDensity* ed_out) {
    ...
    if (bins <= 0) {
        ...
        width = (I > 0
            ? 2.0 * I * std::pow(static_cast<double>(n), -1.0 / 3.0)
            : (b - a) / std::sqrt(static_cast<double>(n)));
    }
    else {
        width = (b - a) / bins;
    }
    if (width <= 0)
        width = (b - a) / std::max(10, static_cast<int>(std::round(std::sqrt(static_cast<double>(n)))));

    int m = std::max(5, static_cast<int>(std::ceil((b - a) / width)));
    width = (b - a) / m;
    ...
    ed.delta = width;
    ed.m = m;
    ed.count = static_cast<int*>(std::malloc(sizeof(int) * m));
    for (int i = 0; i < m; ++i) ed.count[i] = 0;

    for (int i = 0; i < n; ++i) {
        int k = static_cast<int>(std::floor((sample[i] - a) / width));
        if (k < 0)   k = 0;
        if (k >= m)  k = m - 1;
        ed.count[k]++;
    }
...
double empirical_pdf_at_c(const EmpiricalDensity* ed, double x, int n) {
    ...
    int k = static_cast<int>(std::floor((x - ed->a) / ed->delta));
    if (k < 0)   k = 0;
    if (k >= ed->m) k = ed->m - 1;
    return static_cast<double>(ed->count[k]) / (static_cast<double>(n) * ed->delta);
}
