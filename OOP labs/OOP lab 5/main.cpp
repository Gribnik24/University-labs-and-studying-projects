...
void process_sample(const double* sample, int n,
    ...
    std::cout << "  Îáðàáîòêà " << (is_new_sample ? "íîâîé" : "êëàññè÷åñêîé")
        << " âûáîðêè ðàçìåðîì n = " << n << std::endl;
...

    {
        ...
        out.close();
        std::cout << "  Ôàéë " << csv_name << " ñîçäàí" << std::endl;
    }
    ...
    std::cout << "  Òåîðèÿ:   M=" << mm_theor.M << ", D=" << mm_theor.D2
        << ", g1=" << mm_theor.g1 << ", g2=" << mm_theor.g2 << "\n";
    std::cout << "  Ýìïèðèêà: M=" << m_emp.M << ", D=" << m_emp.D2
...
void section_announcement(string header) {
    cout << "\n" << string(70, '=') << "\n";
    cout << header << "\n";
    cout << string(70, '=') << "\n";
}
...
int main() {
    ...
    section_announcement("ÍÀ×ÀËÎ ÒÅÑÒÈÐÎÂÀÍÈß: ËÀÁÎÐÀÒÎÐÍÀß ÐÀÁÎÒÀ #1");
    test_against_table(); // Òåñòû ñðàâíåíèÿ ñ òàáëè÷íûìè çíà÷åíèÿìè
    run_required_tests(); // Òåñòû ïóíêòà 3 èç ËÐ1

    // Âûïîëíåíèå òåñòîâ ãðóïïû 3.3 ñ âûáîðêàìè ðàçíûõ ðàçìåðîâ
    ...
    for (int idx = 0; idx < sample_sizes_count; ++idx) {
        ...
        if (n == 1000) {
            std::cout << "  ==============================\n";
            std::cout << "  === ÂÛÏÎËÍÅÍÈÅ ÒÅÑÒÀ 3.3.2 ===\n";
            ...
            std::cout << "  ÑÐÀÂÍÅÍÈÅ ÂÛÁÎÐÎÊ ÐÀÇÌÅÐÎÌ 1000:" << std::endl;
            std::cout << "  Êëàññè÷åñêàÿ âûáîðêà: M=" << m1.M << ", D=" << m1.D2 << std::endl;
            std::cout << "  Íîâàÿ âûáîðêà:        M=" << m2.M << ", D=" << m2.D2 << std::endl;
            std::cout << "  Ðàçíèöà: deltaM=" << std::fabs(m1.M - m2.M)
    ...
    section_announcement("ÍÀ×ÀËÎ ÒÅÑÒÈÐÎÂÀÍÈß: ËÀÁÎÐÀÒÎÐÍÀß ÐÀÁÎÒÀ #2");
    test_sen_class(); // Òåñòèðîâàíèå êëàññà SENDistribution

    // ==================== ËÀÁÎÐÀÒÎÐÍÀß ÐÀÁÎÒÀ #3 ====================
    section_announcement("ÍÀ×ÀËÎ ÒÅÑÒÈÐÎÂÀÍÈß: ËÀÁÎÐÀÒÎÐÍÀß ÐÀÁÎÒÀ #3");
    test_new_classes(); // Òåñòèðîâàíèå MixtureDistribution è EmpiricalDistribution

    // ==================== ËÀÁÎÐÀÒÎÐÍÀß ÐÀÁÎÒÀ #4 ====================
    section_announcement("ÍÀ×ÀËÎ ÒÅÑÒÈÐÎÂÀÍÈß: ËÀÁÎÐÀÒÎÐÍÀß ÐÀÁÎÒÀ #4");
    test_polymorphism(); // Òåñòèðîâàíèå ïîëèìîðôèçìà

    // ==================== ËÀÁÎÐÀÒÎÐÍÀß ÐÀÁÎÒÀ #5 ====================
    section_announcement("ÍÀ×ÀËÎ ÒÅÑÒÈÐÎÂÀÍÈß: ËÀÁÎÐÀÒÎÐÍÀß ÐÀÁÎÒÀ #5");
    test_link_aggregation();  // Òåñòèðîâàíèå óñòîé÷èâûõ îöåíîê è ïàòòåðíà Íàáëþäàòåëü
