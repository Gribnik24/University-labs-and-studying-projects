...
// Оптимизация вычисления моментов через кэширование
...
// Сеттер. Установка параметра сдвига mu
void SENDistribution::set_mu(double mu) {
    mu_ = mu;
}

// Сеттер. Установка параметра сдвига lambda
void SENDistribution::set_lambda(double lambda) {
    validate_(lambda, v_);
    lambda_ = lambda;
}

// Сеттер. Установка параметра сдвига v
...
// Возвращает плотности распределения по формуле (2)
...
// Создает структуру с математическим ожиданием, дисперсией и коэффициентом эксцесса
...
// Генерация случайного значения из SEN-распределения
double SENDistribution::sample() const {
    return sen_sample(mu_, lambda_, v_);
}

// Реализация IPersistent::save
...
// Реализация IPersistent::load
void SENDistribution::load(std::istream& is) {
    double mu, la, vv;
    if (!(is >> mu >> la >> vv))
...
double SENDistribution::sen_pdf_standard(double x, double v) {
    ...
    const double part_erfc = (v * std::exp(v)) / (2.0 * std::sqrt(2.0)) *
        std::pow(a, -1.5) * std::erfc(sqrt_a);
...
// Вычисляет коэффициент эксцесса стандартного SEN-распределения по формуле (5)
...
SENDistribution load_sen_from_file(const std::string& path) {
    ...
    if (!in)
        throw std::runtime_error("SEN: cannot open file for load: " + path);
    return load_sen_from_stream(in);
}

void save_sen_to_file(const SENDistribution& d, const std::string& path) {
    std::ofstream out(path);
    if (!out)
        throw std::runtime_error("SEN: cannot open file for save: " + path);
    d.save(out);
}
