...
// Конструктор из произвольного распределения
...
// Вычисляет значение эмпирической плотности
double EmpiricalDistribution::pdf(double x) const {
    if (!sample_ || n_ <= 0) return 0.0;
    return empirical_pdf_at_c(&ed_, x, n_);
}

// Вычисляет по выборке математическое ожидание, дисперсию, коэффициенты асимметрии и эксцесса
...
// Возвращает случайный элемент из выборки
...
// Сохраняет размер выборки n_ и все значения выборки по одному числу в строке
...
void EmpiricalDistribution::notify() {
    int count = 0;
...
// Изменение данных
void EmpiricalDistribution::changeData(int index, double newValue) {
    if (index >= 0 && index < n_ && sample_ != nullptr) {
        sample_[index] = newValue;
        build_density();
        notify();
    }
}